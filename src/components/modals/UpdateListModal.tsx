...
export const UpdateListModal = ({
  ...
  const temp = listings.filter((list) => list.id == listId)[0];

  const initial = {
    price: BigNumber.from(temp.price).div(BigNumber.from(10).pow(18)),
    expTime:
  ...
  const updateListingModal = useUpdateListModal();
  const closeModal = () => updateListingModal.onClose();
  ...
  const handleSubmit = async (event) => {
    ...
    if (!pricePerItem || !expirationTime || (tokenType != TokenType.Neander && !quantity)) {
      ...
      updateListing(myParams as any, listId);
  ...
  return (
    <Transition appear show={updateListingModal.isOpen} as={Fragment}>
            ...
            >
              <Dialog.Panel className="relative w-full max-w-md overflow-hidden rounded-lg border-8 border-smolBrown bg-smolBrownAlternative p-3 text-gray-200">
                <div className="mx-auto flex h-full flex-col justify-center p-2">
                  <h2 className="my-2 uppercase">Update List</h2>
